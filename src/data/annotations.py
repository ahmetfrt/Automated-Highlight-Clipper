...
def validate_human_highlights(
    ...
    missing_columns = [
        column for column in REQUIRED_HIGHLIGHT_COLUMNS if column not in highlights
    ]
    if missing_columns:
        raise ValueError(
            f"Human highlights missing required columns: {missing_columns}"
        )
    ...
    if (normalized["end_seconds"] <= normalized["start_seconds"]).any():
        raise ValueError(
            "Every annotation must have end_seconds greater than start_seconds."
        )
...
def _validate_timestamp_consistency(highlights: pd.DataFrame) -> None:
    ...
    if (parsed_start != highlights["start_seconds"]).any():
        raise ValueError(
            "start_time and start_seconds disagree for at least one annotation."
        )
    if (parsed_end != highlights["end_seconds"]).any():
        raise ValueError(
            "end_time and end_seconds disagree for at least one annotation."
        )
...
def _validate_video_ids(highlights: pd.DataFrame, video_registry: pd.DataFrame) -> None:
    ...
    unknown_video_ids = sorted(
        set(highlights["video_id"].astype(str)).difference(known_video_ids)
    )
    if unknown_video_ids:
        raise ValueError(
            f"Annotations reference unknown video_id values: {unknown_video_ids}"
        )


def _validate_annotation_bounds(
    highlights: pd.DataFrame,
    video_registry: pd.DataFrame,
) -> None:
...
def _require_non_empty(frame: pd.DataFrame, columns: list[str]) -> None:
    for column in columns:
        ...
        if frame[column].isna().any() or is_empty.any():
            raise ValueError(
                f"{column} must be present and non-empty for every annotation."
            )
